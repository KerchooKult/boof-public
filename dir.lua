...
client.log('boof.gg has loaded with moonlight obfuscator!')
...
local menuItems = {
    -- Welcome Text
    text = menu.add_text("welcome", "Please shut the fuck up, " .. user.name .. " [".. user.uid .."]"),
    ...
    killSaySetting = menu.add_selection("killsay", "setting", {"boof.gg", "friendly", "anti-ruski","gay","among us","Bunga Mode (200iq)"}),
    -- Halo
    ...
    chat_logs = menu.add_checkbox("indicators", "chat logs"),
    indicator_type = menu.add_selection("indicators", "indicator type", {"none", "circles"}),
}
...
local vars = {
    ...
    stored_cur_time3 = -1, -- crash timer
    killCount = 0, -- session killcount
    circle_x = 500,
    circle_y = 600,
    indicator_size = vec2_t(ref.screen_size.x * 0.1, 6),

    flags_font2 = render.create_font("Consolas", 20, 550, e_font_flags.ANTIALIAS, e_font_flags.OUTLINE),
    ...
    bungaMode = {"erp??> ?","why isnt","jiggy mod me,nu","can i wikihow this?","weewoo gunga","spooky simulator","i get 4 golds as a tank in overwatch. your bitch gives me good head","missed shot due to shooting at head","best cheap headphoe 2020","20 fps","lose if gae","male feminist","if sever if sever ful connect ip player join","blue waffle","mega mom sex free","gamein computer 2020","wher to buy drug","cool segs big tity","gay nigga slaps","mr beast epic challenge","watch video like it subscribe it","bro you fucking suck holy shit","video i make video video","cookiee monstAR snapback","minecrat hypixel free hack no ban","ipod touch","free giftcarrd giveaway 2021 no virus","gamers are awesome 2019","ipad ipad video like free giveaway subscribe","hacking with 500 dollar knife ( tapping skeet )","how to join mexican sex cartel 2020","brake da rules","2b2t geoge floyd cordinate exploit 2015","warcrimes cools","COOKIE MONSER","shut the fuck up 2020","gta 6 free crack","tapping skeet with a prime with my paste in prime in matchmaking with","apple stocks 2021 download","american eagle","how grow banan tree backyar d","apple iphone","free gift car giveaway fre e no virus","razer stock download 2021","sniff that ass like joe biden sniffs kid","how to lower uid free","Tylko jedno w glowie mam koksu 5 gram","parkour tutorial","lake vadeo","how do i become a radio show host","tonka truck real life near me", "71 64 38 15 92 08 67 38 95 08 38 46 90 84 57 24 18 67 40", "ye", "don,da ytmp3.cc album full album download mp3", "gta", "https://quizlet.com/207205174/geometry-1a-final-flash-cards/", "anyone got otc3 update dll kfg unhittable lua?", "verona vitus", "vitamin 字 gummies sale half off.. 2$", "motter moster icecrea", "<字3", "rol", "he lucy bosot  .. boosit", "Unusual Line Termination Detected. Remove\x02029   Ignore", "                             yep", "!rs"},
...
-- Watermark
local motds = {"erp??> ?","why isnt","jiggy mod me,nu","can i wikihow this?","weewoo gunga","spooky simulator","i get 4 golds as a tank in overwatch. your bitch gives me good head","missed shot due to shooting at head","best cheap headphoe 2020","20 fps","lose if gae","male feminist","if sever if sever ful connect ip player join","blue waffle","mega mom sex free","gamein computer 2020","wher to buy drug","cool segs big tity","gay nigga slaps","mr beast epic challenge","watch video like it subscribe it","bro you fucking suck holy shit","video i make video video","cookiee monstAR snapback","minecrat hypixel free hack no ban","ipod touch","free giftcarrd giveaway 2021 no virus","gamers are awesome 2019","ipad ipad video like free giveaway subscribe","hacking with 500 dollar knife ( tapping skeet )","how to join mexican sex cartel 2020","brake da rules","2b2t geoge floyd cordinate exploit 2015","warcrimes cools","COOKIE MONSER","shut the fuck up 2020","gta 6 free crack","tapping skeet with a prime with my paste in prime in matchmaking with","apple stocks 2021 download","american eagle","how grow banan tree backyar d","apple iphone","free gift car giveaway fre e no virus","razer stock download 2021","sniff that ass like joe biden sniffs kid","how to lower uid free","Tylko jedno w glowie mam koksu 5 gram","parkour tutorial","lake vadeo","how do i become a radio show host","tonka truck real life near me"}
local motdNumber = client.random_int(1, #motds)
function on_draw_watermark()
    if(not engine.is_in_game() and not engine.is_connected()) then return end
...
function draw_indicators()
    ...
    if(engine.is_in_game() and menuItems.desyncIndicator:get()) then
        ...
        max_chrage = exploits.get_max_charge()
        can_fire = client.can_fire()
        current_fl = engine.get_choked_commands()
        
        weapon = ref.player:get_active_weapon()
        next_attack = weapon:get_prop("m_flNextPrimaryAttack")
        if not weapon then return end        
        weapon_data = weapon:get_weapon_data()
        if not weapon_data then return end

        if(cur_charge == max_chrage) then
            tick_charge_text = "dt"
            chargeR = 32
            chargeG = 132
            chargeB = 255
        end
        if(cur_charge ~= max_chrage or cur_charge == 0) then
            tick_charge_text = "wait"
            chargeR = 255
            chargeG = 255
            chargeB = 255
        end
        ...
        --]]
        --draggable
        if(menu.is_open() and input.is_key_held(e_keys.MOUSE_LEFT)) then
            user_mouse = input.get_mouse_pos()
        end
        
        if((user_mouse.x - vars.circle_x < 40) and (user_mouse.y - vars.circle_y < 40)) then
            vars.circle_x = user_mouse.x
            vars.circle_y = user_mouse.y
        end
        
        --render.circle(vec2_t.new(vars.circle_x, vars.circle_y), 25, color_t.new(255,0,255,155))
        --draggable

        --below is bubble indicators
        if(menuItems.indicator_type:get() == 2) then
            --double tap
            
            render.circle_filled(vec2_t.new(60, 800), 35, color_t.new(5,5,5,180))
            render.progress_circle(vec2_t.new(60, 800), 35, color_t.new(33,132,255), 2, cur_charge / max_chrage)
            render.text(vars.flags_font2, tick_charge_text, vec2_t.new(61, 800), color_t(chargeR,chargeG,chargeB), true)
            render.circle(vec2_t.new(60, 800), 32, color_t.new(5,5,5, 180))
            --double tap
            
            --fake lag
            render.circle_filled(vec2_t.new(60, 890), 35, color_t.new(5,5,5,155))
            render.progress_circle(vec2_t.new(60, 890), 35, color_t.new(33,132,255), 2, current_fl / ref.fakelag_amount:get())
            render.text(vars.flags_font2, tostring(current_fl), vec2_t.new(45, 891), color_t.new(255,255,255,255), true)
            render.text(vars.flags_font2, "/", vec2_t.new(59, 891), color_t.new(255,255,255,255), true)
            render.text(vars.flags_font2, tostring(ref.fakelag_amount:get()), vec2_t.new(73, 891), color_t.new(255,255,255,255), true)
            render.circle(vec2_t.new(60, 890), 32, color_t.new(5,5,5,255))
            --fake lag
            ...
            render.circle(vec2_t.new(60, 980), 32, color_t.new(5,5,5,255))
            render.circle_filled(vec2_t.new(60, 980), 35, color_t.new(5,5,5,155))
            render.progress_circle(vec2_t.new(60, 980), 35, color_t.new(33,132,255), 2, (weapon:get_prop("m_flNextPrimaryAttack") - global_vars.cur_time()) / weapon_data.cycle_time)
            render.text(vars.flags_font2, can_fire_text, vec2_t.new(61, 981), color_t.new(colorR,colorG,colorB), true)
        end
        --can fire
------------------------------------------------------------------------------------------------------------------------------------------------
        --bar form of above
        if(menuItems.indicator_type:get() == 3) then
            
            charge_fill = map_var(cur_charge, 0, 14) * vars.indicator_size.x
            fakelag_bar_fill = map_var(current_fl, 0, ref.fakelag_amount:get()) * vars.indicator_size.x
            shot_timer_fill = map_var(((weapon:get_prop("m_flNextPrimaryAttack") - global_vars.cur_time()) / weapon_data.cycle_time), 0, 1) * vars.indicator_size.x
            
            -- planet is black lmao
            --double tap (js)
            --base (for niggas)
            render.rect_filled(vec2_t(60, 800), vec2_t(vars.indicator_size.x, 20), color_t( 45, 45, 45, 155), 3)
            -- fill (for niggas with no testosterone)
            render.rect_filled(vec2_t(60, 800), vec2_t(charge_fill, 20),  color_t(0, 255, 0), 3)
            -- outline (for niggas with small balls)
            render.rect(vec2_t(60, 800 - 1), vec2_t(vars.indicator_size.x + 1, 21), color_t(15, 15, 15), 3)
            render.text(vars.flags_font2, "dt", vec2_t.new(60, 790), color_t.new(255,255,255,255), true)
            --double tap
            
            --fake lag (the joke is there is no fakelag)
            --base (for niggas)
            render.rect_filled(vec2_t(60, 890), vec2_t(vars.indicator_size.x, 20), color_t( 45, 45, 45, 155), 3)
            -- fill (for niggas with no testosterone)
            render.rect_filled(vec2_t(60, 890), vec2_t(fakelag_bar_fill, 20),  color_t(0, 255, 0), 3)
            -- outline (for niggas with small balls)
            render.rect(vec2_t(60, 890 - 1), vec2_t(vars.indicator_size.x + 1, 21), color_t(15, 15, 15), 3)
            render.text(vars.flags_font2, "fakelag", vec2_t.new(60, 890), color_t.new(255,255,255,255), true)

            --fake lag 
            
            --checks for shot time(so not stupid)
            if()
            --can fire (..?)
            --base (for niggas)
            render.rect_filled(vec2_t(60, 980), vec2_t(vars.indicator_size.x, 20), color_t( 45, 45, 45, 155), 3)
            -- fill (for niggas with no testosterone)
            render.rect_filled(vec2_t(60, 980), vec2_t(shot_timer_fill, 20),  color_t(0, 255, 0), 3)
            -- outline (for niggas with small balls)
            render.rect(vec2_t(60, 980 - 1), vec2_t(vars.indicator_size.x + 1, 21), color_t(15, 15, 15), 3)
            render.text(vars.flags_font2, "shot timer", vec2_t.new(60, 980), color_t.new(255,255,255,255), true)
...
function killChat(event_info)
    ...
    if(menuItems.killSay:get()) then
        ...
        elseif(menuItems.killSaySetting:get() == 6) then
            s = vars.bungaMode[client.random_int(1, #vars.amongUsMode)]
        else                
            s = vars.boofMode[client.random_int(1, #vars.boofMode)]
        end
...
local function haloSay(event_info)
    ...
    if(menuItems.haloToggle:get()) then
        ...
        if(killNumber == 2) then
            engine.play_sound("2k.wav", 40, 100)
        end
